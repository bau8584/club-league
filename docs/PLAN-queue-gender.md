...
> 상태: **배포됨** (2026-09-15, `deploy-2026-09-15b`). DB 마이그레이션(`2026-09-15_session_gender_mode.sql`) 적용됨. 구현하며 계획과 달라진 점: 바퀴 수는 미지정을 남녀 양쪽에 나눠 붙이는 모든 경우 중 최대로 세고(`gender-split.ts`), 계산기는 같은 조건이면 미지정을 덜 쓰는 조합을 고른다.
