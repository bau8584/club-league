...
// 내가 배정된 '호출된' 대진(입장)을 화면 중앙에 크게 띄운다.
...
export function ScheduledMatchBanner() {
  ...
  const mine = useMemo(() => {
    ...
    return scheduledMatches.filter((m) => {
      ...
      // 오래된(3시간 초과) 호출은 자동 무시 — 남은 행이 매번 다시 뜨는 것 방지
  ...
  // 예약 호출: 팀 미정(참가자 풀). 관리자 대진은 A/B 팀 확정.
  const isReservation = (current.player_ids?.length ?? 0) > 0;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 p-4 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border-2 border-neon-green/60 bg-card shadow-[0_0_60px_rgba(16,185,129,0.35)] animate-in zoom-in-95 duration-300">
        <div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-transparent via-neon-green to-transparent" />
        ...
        <div className="flex flex-col items-center px-6 py-10 text-center sm:px-10 sm:py-12">
          <div className="relative mb-5 flex size-20 items-center justify-center rounded-2xl bg-neon-green/20 text-neon-green">
            <BellRing className="size-10 animate-bounce" />
            <span className="absolute -inset-1.5 rounded-2xl border-2 border-neon-green/40 animate-ping opacity-40" />
          </div>

          <p className="text-[13px] font-black uppercase tracking-[0.25em] text-neon-green">MATCH CALL</p>
          <h2 className="mt-2 text-3xl font-black tracking-tight text-foreground sm:text-4xl">경기 입장!</h2>
          <p className="mt-2 text-sm text-muted-foreground">운영진이 대진을 배정했습니다. 코트로 입장하세요.</p>
